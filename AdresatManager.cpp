...
void AdresatManager::dodajAdresata()
{
    ...
    if (plikZAdresatami.dopiszAdresataDoPliku(adresat))
        ...
        cout << endl << "Nie udalo sie dodac adresata." << endl;
    system("pause");
}
...
int AdresatManager::aktualizujIdOstatniegoAdresata()
{
    idOstatniegoAdresata = plikZAdresatami.aktualizujIdOstatniegoAdresata();
}
...
void AdresatManager::usunAdresata()
{
    ...
    Adresat adresat;
    int nr_linii = 1;
    adresaci = plikZAdresatami.wczytajWszystkichAdresatowZPliku();
    ...
    for (vector <Adresat>::iterator itr = adresaci.begin(); itr != adresaci.end(); itr++)
    {
        if (itr -> pobierzId() == idUsuwanegoAdresata && itr->pobierzIdUzytkownika() == ID_ZALOGOWANEGO_UZYTKOWNIKA)
        {
            ...
            if (znak == 't')
            {
                plikZAdresatami.usunWybranaLinieWPliku(nr_linii);
                adresaci.erase(itr);
...
void AdresatManager::edytujAdresata()
{
    ...
    for (unsigned int i = 0; i < plikZAdresatami.wczytajWszystkichAdresatowZPliku().size(); i++)
    {
        if (adresaci[i].pobierzId() == idEdytowanegoAdresata && adresaci[i].pobierzIdUzytkownika() == ID_ZALOGOWANEGO_UZYTKOWNIKA)
        {
            czyIstniejeAdresat = true;
